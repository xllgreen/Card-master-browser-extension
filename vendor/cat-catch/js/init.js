...
// 初始化当前tabId
const pageTabId = parseInt(new URL(location.href).searchParams.get("tabId"), 10);
function assignActiveTabId(tabs) {
    G.tabId = tabs[0] && tabs[0].id ? tabs[0].id : -1;
}
if (Number.isSafeInteger(pageTabId) && pageTabId > 0) {
    G.tabId = pageTabId;
} else {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
        if (tabs[0] && tabs[0].id) {
            assignActiveTabId(tabs);
            return;
        }
        chrome.tabs.query({ active: true, lastFocusedWindow: true }, function (fallback) {
            assignActiveTabId(fallback);
        });
    });
}
...
G.OptionLists = {
    ...
    catDownload: false, // 始终不启用顺手牵羊下载器
...
function InitOptions() {
    ...
    setTimeout(function () {
        ...
        if (!G.tabId && Number.isSafeInteger(pageTabId) && pageTabId > 0) {
            G.tabId = pageTabId;
        }
    }, 1500);
}
