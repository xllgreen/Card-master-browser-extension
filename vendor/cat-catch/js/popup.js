...
const _embedded = params.get("embedded") === "1";
...
window.addEventListener('message', function (event) {
    if (event.data?.source !== 'card-master-cat-catch' || event.data?.type !== 'page-tab') {
        return;
    }
...
function reportPopupLoadFailure(detail) {
    $tips.html(
        `载入失败 sync=${G.initSyncComplete} local=${G.initLocalComplete} tab=${G.tabId || _tabId || '无'} ${detail || ''}`
    );
}

function mediaListFromResponse(data, tabId) {
    if (Array.isArray(data)) return data;
    if (data && typeof data === "object") {
        const picked = data[tabId] ?? data[String(tabId)];
        if (Array.isArray(picked)) return picked;
    }
    return [];
}

function applyPopupMedia(data) {
    if (!data.length) {
...
function fillPopupMedia(attempt) {
    const tabId = G.tabId || _tabId;
    chrome.runtime.sendMessage(chrome.runtime.id, { Message: "getData", tabId: tabId }, function (data) {
        if (chrome.runtime.lastError || data === "error") {
            if (attempt < 8) {
                setTimeout(function () { fillPopupMedia(attempt + 1); }, 250);
                return;
            }
            reportPopupLoadFailure(chrome.runtime.lastError?.message || data);
            return;
        }
        const list = mediaListFromResponse(data, tabId);
        if (list.length <= 1) {
            chrome.runtime.sendMessage(chrome.runtime.id, { Message: "getAllData" }, function (all) {
                const allList = mediaListFromResponse(all, tabId);
                applyPopupMedia(allList.length > list.length ? allList : list);
            });
            return;
        }
        applyPopupMedia(list);
    });
}

// 一些需要等待G变量加载完整的操作
const popupReadyAt = Date.now();
const interval = setInterval(function () {
    const ready = G.initSyncComplete && G.initLocalComplete;
    if (!ready && Date.now() - popupReadyAt < 400) { return; }
    clearInterval(interval);

    if (G.popup && !_tabId && !_embedded) {
        $("#popup").click();
        return;
    }
    // 侧边面板模式 body 宽度100%
    ...
    chrome.webNavigation?.getAllFrames?.({ tabId: G.tabId }, function (frames) {
        ...
        for (let frame of frames) {
            chrome.tabs.sendMessage(G.tabId, { Message: "getKey" }, { frameId: frame.frameId }, function (result) {
                if (chrome.runtime.lastError || !Array.isArray(result) || result.length === 0) { return; }
                $("#maybeKeyTab").show();
