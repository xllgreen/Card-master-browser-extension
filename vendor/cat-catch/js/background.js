...
chrome.runtime.onMessage.addListener(function (Message, sender, sendResponse) {
    ...
    // 页面内 iframe 优先用消息里的 tabId，其次用所在标签，最后才用后台当前标签
    const requestedTabId = Number(Message.tabId);
    Message.tabId =
        Number.isSafeInteger(requestedTabId) && requestedTabId > 0
            ? requestedTabId
            : (sender.tab?.id ?? G.tabId);
    ...
    if (Message.Message == "getAllData") {
        try {
            sendResponse(JSON.parse(JSON.stringify(cacheData)));
        } catch {
    ...
    if (Message.Message == "getData") {
        const list = cacheData[Message.tabId] ?? [];
        try {
            sendResponse(JSON.parse(JSON.stringify(list)));
        } catch (error) {
            const payload = [];
            for (const item of list) {
                try {
                    payload.push(JSON.parse(JSON.stringify(item)));
                } catch {
                    payload.push({
                        url: item.url,
                        name: item.name,
